...
data = pd.read_pickle('dataset.pkl')
data = data.drop(data.index[0])
owned = pd.read_csv('owned.csv')
owned = owned.columns.values
print(owned)
...
for company in owned:
	try:
		stock  = Sdf.retype(get_stock_data(data, company.upper()))
		...
		plot.title(company.upper())
		plot.savefig(str(company.upper()) + '_owned.png')
		plot.close()
